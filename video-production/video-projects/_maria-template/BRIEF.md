...
1. Hook textual literal en menos de 3 segundos, identificado en el audio o guion y preservado en captions. Se acompaña obligatoriamente de un hook visual creado con al menos un sticker/ilustración real de silueta propia; el punto `1.` de una lista nunca sustituye este beat.
...
3. Ideas importantes apoyadas con gráfico, b-roll, captura, imagen tipo stock o mockup, combinando cuatro formatos: mitad superior, mitad inferior, pantalla completa y `pip-llamada`. No usar un patrón fijo arriba/abajo; ordenar los formatos según el contenido y el ritmo.
   - En V2, el sticker contextual del hook es obligatorio en todos los vídeos. Debe ir grande, llamativo y por delante de la imagen, con impacto frontal de entrada. Durante el desarrollo se pueden intercalar 1-3 stickers adicionales cuando mejoren la comprensión.
   - En el cuerpo, la mayoría de stickers relevantes deben ir grandes pero integrados detrás de la cabeza o del cuerpo mediante recorte/máscara, siempre que el plano lo permita. Usar capa frontal solo si no invade la lectura, la cara ni las zonas seguras.
   - Los stickers deben tener presencia protagonista. No usar stickers mínimos en esquinas como decoración.
   - Un sticker es una ilustración, recorte u objeto con silueta propia; una tarjeta, panel o gráfica no cuenta. Entrada recomendada: `pop`/escala + rotación leve + `wiggle` corto, `0,35–0,65 s`, con SFX sincronizado y salida limpia. Puede usar movimiento tipo GIF finito y determinista.
...
10. En listas, cada numeral abre un caption nuevo. Todo cambio de fondo, layout o fase corta el caption exactamente en el límite, incluso si coincide con el final de una palabra.
11. Registrar en `editorial-structure.json` el hook, sticker obligatorio, requisitos explícitos, enumeraciones y fases; validarlo antes de renderizar.
