# Style Intake â€” How to Interview Without Imposing a Brand
...
â”œâ”€â”€ Yes â†’ Extract values â†’ Use exactly what they supplied
â”œâ”€â”€ Partial (some answers, not all) â†’ Fill gaps with MOTION_PHILOSOPHY defaults, flag the gaps in BRIEF.md
â””â”€â”€ No â†’ Offer MOTION_PHILOSOPHY defaults AND ask for reference videos as a fallback
```
...
| Accent | ONE hex picked for symbolic meaning â€” ask the user "what's the ONE emotion of this video?" and map it |

Keep accent discipline tight. MOTION_PHILOSOPHY Â§0 Law 7: â‰¤5 symbolic colors across the whole video, each with a named meaning.
...
1. **Ask for at least three roles:**
   ...
   - (Optional: surface Â· border Â· warn)

2. **Enforce the â‰¤5 symbolic colors rule.** If they give you 8 colors, ask which three carry narrative work and which five are decorative â€” drop the decorative.
...
4. **Check contrast.** If accent hex on background hex is below 4.5:1 for text-sized elements, flag it to the user. Text on a dark background often needs a `text-shadow` halo to read well â€” MOTION_PHILOSOPHY Â§2.2 covers the recipe.

## When the user supplies fonts

1. Resolve Google Fonts names â†’ `<link>` tags. If a font is custom, capture the file path and plan a `@font-face` declaration.
...
1. Ask for: file path Â· clearspace preference Â· any known glow/effect spec
2. In `style-profile.md` record: path Â· dimensions Â· CSS filter recipe (e.g. `drop-shadow(0 0 40px rgba(accent, 0.6))` if they want a glow)
...
2. Extract:
   - Dominant palette (2â€“4 hex codes)
   - Type feel (serif Â· sans Â· mono Â· display)
   - Pacing (kinetic Â· balanced Â· relaxed)
   - Primary motion vocabulary (whip-pans Â· morphs Â· fades Â· glitches Â· 3D reveals)
3. Propose a derived palette + font pair + pacing. **Confirm with the user** before building.

---

## `<project-folder>/assets/style-profile.md` â€” what it contains

```markdown
# Style Profile â€” <project-slug>
...
- 1â€“2 second scenes in the body of the video
- One idea per beat
- â‰¤5 symbolic colors, each with a named meaning
- Grid / vignette / grain texture on every scene
- Motion-blur transitions (whip-pans, morphs) â€” never hard cuts
- Hold the outro 4â€“6 seconds
