# Catalog Intent Map â€” "User Says X â†’ Install Y"
...
| User saysâ€¦ | Install |
|---|---|
| "title card / opener / kinetic type" | hand-build â€” registry has no kinetic-type block; see MOTION_PHILOSOPHY Â§3.3 |
...
| "smooth transition / soft fade" | `transitions-blur` Â· `transitions-dissolve` |
| "energetic transition / fast cut / punchy" | `whip-pan` Â· `cinematic-zoom` Â· `glitch` |
| "cinematic transition / premium morph" | `cross-warp-morph` Â· `swirl-vortex` |
| "act break / dramatic flash" | `flash-through-white` Â· `light-leak` |
| "zoom-through text" | `cinematic-zoom` |
| "color change on same scene" | `chromatic-radial-split` â€” or hand-build the CSS-variable recolor in MOTION_PHILOSOPHY Â§3.5 |
...
| "grid tile transition / pixelate wipe" | `grid-pixelate-wipe` (component) Â· `transitions-grid` |
...
4. **Wire into `index.html`:**
   ...
   ```
5. **Apply the user's palette.** Most blocks accept CSS variables at the top of the file â€” find them and override with the user's hex codes from `style-profile.md`.

---

## Components vs. blocks

- **Blocks** are standalone sub-compositions â€” their own timeline, their own root `<div>`. Reference via `data-composition-src`. Examples: `logo-outro`, `app-showcase`, `tiktok-follow`.
...
Catalog blocks buy production velocity. When you have time, hand-building hero moments (kinetic-type openers, logo reveals) usually looks better than installing a block â€” the MOTION_PHILOSOPHY reference spot installs **zero** blocks.

**Rule of thumb for this skill:**
- Transitions and social/UI overlays â†’ install
- Hero opener and final CTA â†’ hand-build unless the user is on a tight deadline
- Film grain and shimmer â†’ always install (they're components, meant to be dropped in)

---

## Gotchas

- After installing a block, always run `npx hyperframes lint` â€” the installer can create `data-composition-id` collisions if you already have one with the same name.
...
- Some blocks (notably `logo-outro`) expect specific asset filenames â€” read the installed file and rename / re-path to the user's logo.
