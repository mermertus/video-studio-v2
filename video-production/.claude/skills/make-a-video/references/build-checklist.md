...
- [ ] Captions are body-level siblings in `index.html` (not inside sub-compositions), each with `data-track-index â‰¥ 20`
- [ ] No `Math.random()`, no `Date.now()`, no render-time `fetch()`
- [ ] Catalog block CSS is scoped under `[data-composition-id="..."]` (no leaked `html, body { ... }`)
- [ ] `npx hyperframes lint` â€” all errors fixed, warnings triaged

---

## Preflight â€” aesthetic

- [ ] Palette on-brief â€” no rogue colors, â‰¤5 symbolic hues
- [ ] Typography on-brief â€” only the fonts declared in `assets/style-profile.md`
...
- [ ] Every scene transition is a *motion* transition (whip Â· morph Â· blur Â· recolor), not a hard fade
- [ ] Outro holds 4â€“6 seconds
- [ ] At least one visual callback (element returns later)
- [ ] Scene lengths follow the brief's pacing tier (kinetic 1â€“2s Â· balanced 2â€“3s Â· relaxed 3â€“5s)

---

## PREVIEW GATE 1 â€” Studio
...
- [ ] **Explicit "looks good, render a draft" captured** â€” silence is not approval
...
- [ ] **Every PNG opened via the `Read` tool** â€” not just listed by filename
...
- [ ] Any regression â†’ fixed â†’ re-rendered â†’ re-verified

---

## PREVIEW GATE 2 â€” Rendered MP4

- [ ] `npx serve renders -p 8080 -n` running (NOT Python's `http.server` â€” no Range support breaks scrubbing)
- [ ] User given `http://localhost:8080/<slug>-draft.mp4`
- [ ] Playback scrubs cleanly end-to-end
- [ ] Audio syncs with visuals (voiceover Â· SFX Â· music bed)
...
- [ ] File exists, size reasonable for duration (roughly 3â€“10 MB per 10 seconds at 1080p)
...
- [ ] Offer one round of revisions â€” the user almost always has one
- [ ] If the user wants a different format (e.g. 1:1 crop from 16:9), plan that as a new pass through Gates 1â€“8 with the original `BRIEF.md` as the starting point
