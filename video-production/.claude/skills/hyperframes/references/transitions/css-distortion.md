...
RGB-tinted overlays (NOT multiply blend â€” use normal blending at 35% opacity) jitter with large offsets. Scene itself also jitters.
...
tl.set(old, { x: -15 }, T);
// 6 jitter frames at 0.03s intervals with big offsets (Â±30-60px)
// ... swap and clear at T + 0.2
```

### Chromatic Aberration

RGB overlays start aligned then spread apart (Â±80px), scene fades, converge on new scene.
...
Rapid oscillation (Â±30px) + scale distortion (0.97-1.03) + increasing blur. Swap at peak distortion.
