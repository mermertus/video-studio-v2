...
The compiler embeds supported fonts â€” just write `font-family` in CSS.
...
- **Don't pair two sans-serifs.** You do this constantly â€” one for headlines, one for body. Cross the boundary: serif + sans, or sans + mono.
...
- **Tension should mean something.** Don't pattern-match pairings. Ask WHY these two fonts disagree. The pairing should embody the content's contradiction â€” mechanical vs human, public vs private, institutional vs personal. If you can't articulate the tension, it's arbitrary.
- **Register switching.** Assign different fonts to different communicative modes â€” one voice for statements, another for data, another for attribution. Not hierarchy on a page. Voices in a conversation.
...
- **Motion is typography.** How a word enters carries as much meaning as the font. A 0.1s slam vs a 2s fade â€” same font, completely different message.
...
# Trending Sans â€” recent (2022+), popular (<300)
R["Trending Sans"] = []
for f in fonts:
...
# Trending Serif â€” recent (2018+), popular (<600)
R["Trending Serif"] = []
for f in fonts:
    if not ok(f) or f["family"] in seen: continue
...
# Monospace â€” recent (2018+), popular (<600)
R["Monospace"] = []
for f in fonts:
    if not ok(f) or f["family"] in seen: continue
...
# Impact & Condensed â€” heavy display fonts with 800+ weight
R["Impact & Condensed"] = []
for f in fonts:
    if not ok(f) or f["family"] in seen: continue
...
# Script & Handwriting â€” popular (<300)
...
Five categories: trending sans, trending serif, monospace, impact/condensed, script/handwriting. All dynamically filtered from Google Fonts metadata â€” no hardcoded font names. Cross classification boundaries when pairing.

## Selection Thinking

Don't pick fonts by category reflex (editorial â†’ serif, tech â†’ mono, modern â†’ geometric sans). That's pattern matching, not design.
...
```css
/* Tabular numbers â€” digits align vertically in columns */
.stat-value,
.timer,
.data-column {
...
/* Diagonal fractions â€” renders 1/2 as Â½ */
.recipe-amount,
.ratio {
  font-variant-numeric: diagonal-fractions;
}

/* Small caps for abbreviations â€” less visual shouting */
.abbreviation,
.unit {
  font-variant-caps: all-small-caps;
}

/* Disable ligatures in code â€” fi, fl, ffi should stay separate */
code,
.code {
...
`tabular-nums` is essential any time numbers are stacked vertically â€” stat callouts, timers, scoreboards, data tables. Without it, digits have proportional widths and columns don't align.
