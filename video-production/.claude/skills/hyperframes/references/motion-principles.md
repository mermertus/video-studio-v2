...
- **Don't use the same ease on every tween.** You default to `power2.out` on everything. Vary eases like you vary font weights â€” no more than 2 independent tweens with the same ease in a scene.
- **Don't use the same speed on everything.** You default to 0.4-0.5s for everything. The slowest scene should be 3Ã— slower than the fastest. Vary duration deliberately.
...
**Direction rules â€” these are not optional:**
...
- Fast (0.15-0.3s) â€” energy, urgency, confidence
- Medium (0.3-0.5s) â€” professional, most content
- Slow (0.5-0.8s) â€” gravity, luxury, contemplation
- Very slow (0.8-2.0s) â€” cinematic, emotional, atmospheric
...
- **Build (0-30%)** â€” elements enter, staggered. Don't dump everything at once.
- **Breathe (30-70%)** â€” content visible, alive with ONE ambient motion.
- **Resolve (70-100%)** â€” exit or decisive end. Exits are faster than entrances.
...
The element that moves first is perceived as most important. Stagger in order of importance, not DOM order. Don't wait for completion â€” overlap entries. Total stagger sequence under 500ms regardless of item count.
