...
3. **Timing** â€” which clips drive the duration, where do transitions land, what's the pacing.
4. **Layout** â€” build the end-state first. See "Layout Before Animation" below.
5. **Animate** â€” then add motion using the rules below.
...
1. **DESIGN.md exists in the project?** â†’ Read it. Use its exact colors, fonts, motion rules, and "What NOT to Do" constraints.
2. **visual-style.md exists?** â†’ Read it. Apply its `style_prompt_full` and structured fields. (Note: `visual-style.md` is a project-specific file. `visual-styles.md` is the style library with 8 named presets â€” different files.)
3. **User named a style** (e.g., "Swiss Pulse", "dark and techy", "luxury brand")? â†’ Read [visual-styles.md](./visual-styles.md) for the 8 named presets. Generate a minimal DESIGN.md with: `## Style Prompt` (one paragraph), `## Colors` (3-5 hex values with roles), `## Typography` (1-2 font families), `## What NOT to Do` (3-5 anti-patterns).
4. **None of the above?** â†’ Ask 3 questions before writing any HTML:
...
Every composition must trace its palette and typography back to a DESIGN.md, visual-style.md, or explicit user direction. If you're reaching for `#333`, `#3b82f6`, or `Roboto` â€” you skipped this step.
</HARD-GATE>

For motion defaults, sizing, entrance patterns, and easing â€” follow [house-style.md](./house-style.md). The house style handles HOW things move. The DESIGN.md handles WHAT things look like.
...
3. **Add entrances with `gsap.from()`** â€” animate FROM offscreen/invisible TO the CSS position. The CSS position is the ground truth; the tween describes the journey to get there.
...
- **Fonts:** Just write the `font-family` you want in CSS â€” the compiler embeds supported fonts automatically. If a font isn't supported, the compiler warns.
...
- **Dead zones**: periods over 1s with no animation â€” intentional hold or missing entrance?
...
Read the JSON. Scan summaries for anything unexpected. Check every flag â€” fix or justify. Verify the timeline shows the intended choreography rhythm. Re-run after fixes.
...
- **[references/sfx-sync.md](references/sfx-sync.md)** â€” SFX sincronizados al frame: fÃ³rmula `data-start = peak âˆ’ transient_offset`, librerÃ­a `assets/sfx/` + manifest, mapeo cueâ†’SFX. Read when adding sound effects timed to transitions/beats.
- **[references/audio-layers.md](references/audio-layers.md)** â€” Capas de audio (VO/SFX/mÃºsica via `data-layer`), modo step-by-step con render por etapa (`stage-render.mjs`), y mÃºsica multi-track horneada con fades (`music-bed.mjs` + `music-plan.json`). Read when working in step-by-step mode or adding background music.
- **[references/css-patterns.md](references/css-patterns.md)** â€” CSS+GSAP marker highlighting: highlight, circle, burst, scribble, sketchout. Deterministic, fully seekable. Read when adding visual emphasis to text.
- **[references/typography.md](references/typography.md)** â€” Typography: font pairing, OpenType features, dark-background adjustments, font discovery script. **Always read** â€” every composition has text.
...
- **[visual-styles.md](visual-styles.md)** â€” 8 named visual styles (Swiss Pulse, Velvet Standard, Deconstructed, Maximalist Type, Data Drift, Soft Signal, Folk Frequency, Shadow Cut) with hex palettes, GSAP easing signatures, and shader pairings. Read when user names a style or when generating DESIGN.md.
...
- **[references/transitions.md](references/transitions.md)** â€” Scene transitions: crossfades, wipes, reveals, shader transitions. Energy/mood selection, CSS vs WebGL guidance. **Always read for multi-scene compositions** â€” scenes without transitions feel like jump cuts.
  ...
  - Shader transitions are in `@hyperframes/shader-transitions` (`packages/shader-transitions/`) â€” read package source, not skill files.
