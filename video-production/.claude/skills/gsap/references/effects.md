...
All effects follow HyperFrames composition rules â€” deterministic, no randomness, timelines registered via `window.__timelines`.
...
1. **One cursor visible at a time** â€” hide previous before showing next.
2. **Cursor must blink when idle** â€” after typing, during pauses.
3. **No gap between text and cursor** â€” elements must be flush in HTML.
...
Pattern: blink â†’ solid (typing starts) â†’ type â†’ solid â†’ blink (typing done).
...
TextPlugin removes from front â€” wrong for backspace. Use manual substring removal:
...
Type â†’ hold â†’ backspace â†’ next word. Cursor blinks during every idle moment (holds, after backspace).
...
When handing off between typewriter lines: hide previous â†’ blink new â†’ pause â†’ solid when typing. Never go hiddenâ†’solid (skips idle state).
...
// Option B: sync XHR (large files â€” must be synchronous for deterministic timeline construction)
...
**Do NOT use async `fetch()` to load audio data.** HyperFrames requires synchronous timeline construction â€” the capture engine reads `window.__timelines` synchronously after page load. Building timelines inside `.then()` callbacks means the timeline isn't ready when capture starts.

### Rendering Approaches

**Canvas 2D** (most common â€” bars, waveforms, circles, gradients):
...
**WebGL / Three.js** â€” HyperFrames patches `THREE.Clock` for deterministic time. Update uniforms from audio data each frame.

**DOM Elements** â€” fine for < 20 elements, less performant than Canvas for many.
...
- **Bass drives big moves** â€” scale, glow, position shifts
- **Treble drives detail** â€” shimmer, flicker, edge effects
- **RMS drives globals** â€” background brightness, overall energy
- Pick 2-3 properties to animate. More looks noisy.
- Keep minimums above zero â€” quiet sections need life.
...
Layer multiple canvases with CSS z-index for depth â€” a background layer driven by bass/rms and a foreground layer driven by individual bands creates depth without complexity.
