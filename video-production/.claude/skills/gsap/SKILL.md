...
- **ease** â€” `"power1.out"` (default), `"power3.inOut"`, `"back.out(1.7)"`, `"elastic.out(1, 0.3)"`, `"none"`.
- **stagger** â€” number `0.1` or object: `{ amount: 0.3, from: "center" }`, `{ each: 0.1, from: "random" }`.
- **overwrite** â€” `false` (default), `true`, or `"auto"`.
- **repeat** â€” number or `-1` for infinite. **yoyo** â€” alternates direction with repeat.
- **onComplete**, **onStart**, **onUpdate** â€” callbacks.
- **immediateRender** â€” default `true` for from()/fromTo(). Set `false` on later tweens targeting the same property+element to avoid overwrite.
...
- **autoAlpha** â€” prefer over `opacity`. At 0: also sets `visibility: hidden`.
- **CSS variables** â€” `"--hue": 180`.
- **svgOrigin** _(SVG only)_ â€” global SVG coordinate space origin. Don't combine with `transformOrigin`.
- **Directional rotation** â€” `"360_cw"`, `"-170_short"`, `"90_ccw"`.
- **clearProps** â€” `"all"` or comma-separated; removes inline styles on complete.
- **Relative values** â€” `"+=20"`, `"-=10"`, `"*=2"`.
...
Built-in eases: `power1`â€“`power4`, `back`, `bounce`, `circ`, `elastic`, `expo`, `sine`. Each has `.in`, `.out`, `.inOut`.
...
- **Absolute**: `1` â€” at 1s
- **Relative**: `"+=0.5"` â€” after end; `"-=0.2"` â€” before end
- **Label**: `"intro"`, `"intro+=0.3"`
- **Alignment**: `"<"` â€” same start as previous; `">"` â€” after previous ends; `"<0.2"` â€” 0.2s after previous starts
...
- **repeat**, **yoyo** â€” apply to whole timeline.
...
- **[references/effects.md](references/effects.md)** â€” Drop-in effects: typewriter text, audio visualizer. Read when needing ready-made effect patterns for HyperFrames.
...
- Skip cleanup â€” always kill tweens when no longer needed.
