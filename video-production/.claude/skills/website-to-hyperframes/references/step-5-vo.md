...
- **Kokoro** (try first â€” free, no API key) â€” `npx hyperframes tts SCRIPT.md --voice af_nova --output narration.wav`. Runs locally on CPU. Requires Python 3.10+ (macOS system Python 3.9 won't work â€” if it fails with an onnxruntime error, move to the next option).
- **ElevenLabs** (best voice quality, widest selection) â€” `mcp__elevenlabs__search_voices` to browse, `mcp__elevenlabs__text_to_speech` to generate. Does not return timestamps â€” transcribe separately after.
- **HeyGen TTS** (returns word timestamps automatically â€” saves a transcribe step) â€” `mcp__claude_ai_HeyGen__text_to_speech`. Use when you want timestamps without a separate transcription pass.

Pick the voice that sounds most natural and conversational. Listen for pacing â€” does it breathe between sentences? Does it sound like a person or a robot?
...
Beat boundaries land on word onsets â€” hard cuts to the VO.
