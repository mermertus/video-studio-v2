...
`lint` checks HTML structure statically â€” missing attributes, timeline registration, tween conflicts, CSS transform + GSAP conflicts (including inline styles).
...
This renders one frame per beat at the moment when content is most visible. Use timestamps where the most content is on screen â€” usually 60-70% into each beat, after entrances finish but before exits start.
...
- For each beat, check: does the snapshot show the assets you referenced in the HTML? If a composition has `<img src="...wave.png">` but the snapshot shows no wave â€” the image isn't loading, the path is wrong, or it's hidden behind another element.
...
- The snapshot command is fast â€” run it multiple times at different timestamps if needed.
