...
These are NOT advanced â€” they're standard motion design patterns that every composition should use at least 2-3 of.
...
Animated noise, particle fields, data visualizations â€” anything that evolves frame-by-frame. Drive it with a GSAP proxy.
...
The `hash()` function is deterministic â€” same frame renders identically every time.
...
The slide distance DECAYS per word (80â†’12px) â€” mimics a camera settling.
...
<script>
  // Video playback is controlled by the framework â€” don't call play() manually
...
Use `ease: "steps(1)"` for cursor blink â€” creates discrete on/off.
...
The glyph subtly reshapes as axes animate â€” optical size adjusts detail, weight changes thickness.
...
Exit one beat and enter the next with matched velocities â€” creates perceived continuous motion.
...
The fastest point of both curves meets at the cut â€” the viewer perceives smooth camera motion. Match ease families: `.in` for exits, `.out` for entries.
