...
# Contrato editorial obligatorio: hook, listas, fases y stickers

- Todo vídeo abre con un **hook textual** antes de cualquier enumeración. Si está pronunciado, el caption debe reproducirlo literalmente; el hook visual lo acompaña, no lo sustituye.
- Todo vídeo incluye al menos un **sticker real como hook visual**, creado para el concepto y con movimiento finito tipo GIF. Un sticker real es una ilustración, objeto o recorte con silueta propia; una tarjeta, panel, gráfico rectangular o texto dentro de una caja no cuenta como sticker.
- Un numeral de lista nunca comparte caption con la última palabra del punto anterior. Cada `1.`, `2.`, `3.`… inicia un caption nuevo y puede reforzarse como número grande durante unos segundos.
- Todo cambio de fondo, layout, contraste o fase fuerza un corte de caption. La frontera también se aplica cuando su timestamp coincide exactamente con el final de una palabra.
- Las peticiones visuales explícitas del brief son requisitos bloqueantes de producción y QA.
- En QA, cada transición se inspecciona en tres frames: uno antes, el frame límite y uno después.
- Antes del render se ejecuta `node scripts/validate-editorial-structure.mjs <project-folder>`.
